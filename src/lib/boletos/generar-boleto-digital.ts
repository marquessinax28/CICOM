...
import { generarFolio, generarPasswordBoleto, hashPasswordBoleto } from "@/lib/hash";
...
import { enviarBoletoDigital } from "@/lib/resend";

// Se llama SOLO desde el webhook de Stripe, justo después de que
...
// evento). Genera folio + contraseña, los inserta (hasheados) vía
// fn_crear_boleto_digital, arma el PDF con el monto YA CONGELADO en la
// orden (nunca vuelve a resolver el precio vigente) y lo entrega por las
// dos vías que pide CLAUDE.md: lo sube a un bucket privado (para la
// descarga inmediata desde la página de éxito, siempre por URL firmada de
// vida corta -- nunca una ruta pública) y lo manda por correo con Resend.
//
