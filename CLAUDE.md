...
- **Contraseñas de boleto:**
  ...
  - Guarda **solo el hash**. Implementación real: `bcryptjs` (`src/lib/hash-nucleo.ts`) — se eligió sobre Argon2id porque es JS puro sin bindings nativos, requisito para compilar en las funciones serverless de Vercel; Argon2id necesita un addon nativo que no siempre está disponible en ese runtime. **Costo 10, no 12** (bajado 2026-09-05): a diferencia del código de verificación y la contraseña de admin (que se hashean una a la vez), la contraseña de boleto se hashea en LOTES de cientos al generar boletos físicos, secuencial -- bcryptjs no reparte ese trabajo entre hilos reales (confirmado midiendo: paralelizar con `Promise.all` no bajó el tiempo). A costo 12, 500 hashes tardaban ~112s y provocaron un `FUNCTION_INVOCATION_TIMEOUT` real en Vercel; a costo 10, ~28s. El espacio de contraseñas no cambia (32⁸ combinaciones); baja únicamente el costo de cada intento de fuerza bruta *offline* (solo relevante si la base se filtra) -- la defensa contra fuerza bruta en línea sigue siendo el bloqueo progresivo + Turnstile, que no dependen del costo del hash. Los hashes ya emitidos con costo 12 no se tocan ni necesitan migrarse: bcrypt guarda el costo dentro del propio string del hash, así que ambos costos conviven en la misma columna sin problema. Nunca en claro, nunca cifrada de forma reversible, nunca recuperable — solo reemitible.
...
- [ ] Contraseñas de boleto hasheadas con bcrypt cost 10 (`bcryptjs`; admin y código de verificación se quedan en cost 12 -- ver sección 2); ninguna en claro en la BD.
